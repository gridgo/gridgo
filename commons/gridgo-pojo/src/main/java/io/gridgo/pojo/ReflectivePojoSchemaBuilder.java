...
public class ReflectivePojoSchemaBuilder implements PojoSchemaBuilder {
    ...
    public PojoSchema build(@NonNull Class<?> type) {
        ...
        var getters = new HashMap<String, List<PojoGetter>>();
        ...
        for (var method : methods) {
            ...
            if (method.getParameterCount() == 0) {
                ...
                if (pair == null)
                    continue;
                getters.computeIfAbsent(pair.getLeft(), k -> new LinkedList<>()).add(pair.getRight());
            ...
            case ANY:
                if (field.trySetAccessible()) {
                    ...
                    if (fieldAccessMode.isGetable())
                        getters.computeIfAbsent(fieldName, k -> new LinkedList<>()).add(PojoGetter.compile(field));
